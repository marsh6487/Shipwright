...
inline uint32_t ReadLE32(const Bytes& b, size_t p) {
    return (uint8_t)b[p] | (uint32_t)(uint8_t)b[p+1]<<8 | (uint32_t)(uint8_t)b[p+2]<<16 | (uint32_t)(uint8_t)b[p+3]<<24;
}
...
inline bool Wrapper(const Bytes& b, unsigned pose) {
    if (pose > 1) return false;
    const char* path = ClipPath(pose); size_t length = strlen(path);
...
struct LimbSignature { const char* name; int16_t pos[3]; uint8_t child, sibling; const char* dl; };
...
inline std::string Path(const char* name) { return std::string("objects/object_test3/") + name; }
inline bool SkeletonBytes(const Bytes& b) {
    if (b.size() != 983 || !Header(b,0x4F534B4C) || b[64] != 1 || b[65] != 1 || ReadLE32(b,66) != 21 ||
        ReadLE32(b,70) != 18 || b[74] != 1 || ReadLE32(b,75) != 21) return false;
    size_t at = 79;
    for (const auto& limb : Limbs) {
        std::string path = Path(limb.name);
        if (at+4 > b.size() || ReadLE32(b,at) != path.size()) return false;
        at += 4;
        if (path.size() > b.size()-at || memcmp(b.data()+at,path.data(),path.size())) return false;
        at += path.size();
    }
    return at == b.size();
}
inline bool LimbBytes(const Bytes& b, unsigned index) {
    if (index >= 21 || !Header(b,0x4F534C42) || b.size() < 122 || b[64] != 2) return false;
    for (size_t i=65;i<106;++i) if (b[i] != 0) return false;
    const auto& expected = Limbs[index];
    std::string dl = *expected.dl ? Path(expected.dl) : "";
    size_t at=106;
    for (int i=0;i<2;++i) {
        if (at+4 > b.size() || ReadLE32(b,at) != dl.size()) return false;
        at += 4;
        if (dl.size() > b.size()-at || memcmp(b.data()+at,dl.data(),dl.size())) return false;
        at += dl.size();
    }
    if (b.size() != at+8) return false;
    for (int i=0;i<3;++i) {
        uint16_t v=(uint8_t)b[at+i*2] | (uint16_t)(uint8_t)b[at+i*2+1]<<8;
        if ((int16_t)v != expected.pos[i]) return false;
    }
...
inline bool Limb(const std::shared_ptr<SOH::SkeletonLimb>& resource, unsigned index) {
    if (!resource || index >= 21 || resource->limbType != SOH::LimbType::LOD) return false;
    const auto& expected = Limbs[index]; const auto& limb = resource->limbData.lodLimb;
    if (limb.jointPos.x != expected.pos[0] || limb.jointPos.y != expected.pos[1] || limb.jointPos.z != expected.pos[2] ||
        limb.child != expected.child || limb.sibling != expected.sibling) return false;
