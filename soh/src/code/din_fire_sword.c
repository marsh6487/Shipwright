...
static void DinFireSword_DrawLayers(PlayState* play, s32 profile, u16 phase) {
    Gfx* coreDL;
    Gfx* flameDL;
    if (!DinFireSword_Load(profile, &coreDL, &flameDL))
        return;
...
void DinFireSword_Draw(PlayState* play, Player* player) {
    ...
    if (!DinFireSword_SameContext(play, player))
        return;
    DinFireSword_DrawLayers(play, DinFireSword_Profile(player), sSword.phase);
}
...
void DinFireSword_DrawPedestal(PlayState* play) {
    if (play == NULL || !CVarGetInteger(CVAR_ENHANCEMENT("DinFireSword"), 0) ||
        ...
        return;
    DinFireSword_DrawLayers(play, LINK_AGE_ADULT, play->gameplayFrames & 1023);
}
