...
int ResourceMgr_OTRSigCheck(char* value) { return 0; }
...
static void RequireLevelHead(Vec3s* joints) {
    ...
    for (int n = 0; n < 3; ++n) {
        ...
        Rotate(&joints[joint], forward); Rotate(&joints[joint], up);
    }
...
int main(void) {
    ...
    StaticStoryKokiri_Init(&girl, play); StaticStoryKokiri_Init(&fado, play);
    ...
    RequireLevelHead(girlJoints); RequireLevelHead(fadoJoints);
    ...
    for (int tick = 0; tick < fixtureDonor.common.frameCount * 4 + 3; ++tick) {
        ...
        Update(&girl, 0.5f); Update(&fado, 0.5f);
    ...
    free(play);
    puts("PASS both characters: level heads from init, donor leg motion, two loops, fractional seam, independent restart, unchanged other poses and missing-donor fallback");
    return 0;
}
