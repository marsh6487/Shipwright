...
int main() {
 BeginFrame(0x5b,9,0,1,true,true);
 {
  ...
  frame.lookups++;
  MetadataRead read("C:\\mods\\room11.o2r"); read.SetBytes(4706089);
 }
 std::thread worker([] { MetadataRead read("worker.o2r");read.SetBytes(99); }); worker.join();
 BeginRender(11,9);
 auto result=EndFrame();require(result.has_value());
 ...
 BeginFrame(0x5b,11,9,2,true,true);BeginRender(11,9);require(!EndFrame().has_value());
 ...
 std::thread crossingWorker([&] {
  ...
  MetadataRead read("crossing-worker.o2r");read.SetBytes(123);
 ...
 crossingWorker.join();
 result=EndFrame();require(result.has_value());
 ...
 std::thread invalidator([] { metadataGeneration.fetch_add(1, std::memory_order_relaxed); }); invalidator.join();
 result=EndFrame();require(result.has_value());
 require((*result)["metadata_invalidations_during_frame"]==1 && (*result)["metadata_reads"]==0);
 BeginFrame(0x5b,11,9,5,true,false);{MetadataRead read("disabled");read.SetBytes(1);}require(!EndFrame().has_value());
 BeginFrame(0x51,11,9,6,true,true);{MetadataRead read("other scene");read.SetBytes(1);}require(!EndFrame().has_value());
 BeginFrame(0x5b,11,9,7,true,true);result=EndFrame();require(result.has_value());
 require((*result)["all_threads_completed_work"]["metadata_reads"]==0);
 BeginFrame(0x5b,11,9,8,true,true);require(!EndFrame().has_value());
 ...
 {
  Scope lookup(&Frame::lookupNanos);lookup.MarkCacheHit();frame.lookups++;frame.cacheHits++;
  MetadataRead read("reload-main.o2r");read.SetBytes(10);
 }
 std::thread reloadWorker([] {
  Scope lookup(&Frame::lookupNanos);lookup.MarkCacheMiss();
  Scope parse(&Frame::parseNanos);
  Scope decode(&Frame::decodeNanos);
  MetadataRead read("reload-worker.o2r");read.SetBytes(20);
 });
 reloadWorker.join();
 result=EndStateReload(0x52,4);require(result.has_value());
 ...
 BeginStateReload(0x51,2,true,true);{MetadataRead read("not-lost-woods");read.SetBytes(1);}
 require(!EndStateReload(0x5b,9).has_value());
 BeginStateReload(0x5b,9,true,false);{MetadataRead read("disabled-reload");read.SetBytes(1);}
 require(!EndStateReload(0x52,4).has_value());
 std::cout<<"PASS main-thread attribution, all-thread completed work, state reload interval, room tags, frame reset, quiet warm frames, disabled mode and scene filter\n";
}
