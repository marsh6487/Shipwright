...
static struct { const char* dlists[2]; } sDrawItemTable[256] = {
...
int32_t CVarGetInteger(const char* name, int32_t fallback) {
    if (!strcmp(name, "gCosmetics.Consumable.Hearts.Changed")) return sHeartsChanged;
    if (!strcmp(name, "gCosmetics.Consumable.Magic.Changed")) return sMagicChanged;
    return fallback;
}
Color_RGB8 CVarGetColor24(const char* name, Color_RGB8 fallback) {
    if (!strcmp(name, "gCosmetics.Consumable.Hearts.Value")) return sHeartsColor;
    if (!strcmp(name, "gCosmetics.Consumable.Magic.Value")) return sMagicColor;
    return fallback;
}
uint8_t ResourceGetIsCustomByName(const char* name) { return sCustom; }
void gSPDisplayList(Gfx* pkt, Gfx* dl) {
    ...
    if (sFirstAltDL && !strcmp((const char*)dl, sFirstAltDL)) sCustom = 1;
    __gSPDisplayList(pkt, dl);
}
void Gfx_SetupDL_25Opa(GraphicsContext* gfx) {}
void Gfx_SetupDL_25Xlu(GraphicsContext* gfx) {}
Mtx* Matrix_NewMtx(GraphicsContext* gfx, char* file, s32 line) { return &sMatrix; }
...
static void CheckStreamWithBorder(const char* label, Gfx* begin, Gfx* end, const char* body, int tint,
                                 Color_RGB8 color, int whiteBorder) {
    int gray = 0, bodySeen = 0, borderSeen = 0, error = 0;
    uint32_t rgba = 0;
    for (Gfx* g = begin; g < end; ++g) {
        unsigned op = (g->words.w0 >> 24) & 255;
        if (op == G_SETINTENSITY) rgba = g->words.w1;
        if (op == G_SETGRAYSCALE) gray = g->words.w1;
        if (op == G_DL) {
            const char* path = (const char*)g->words.w1;
            if (!strcmp(path, body)) {
                ++bodySeen;
                error |= gray != tint;
                if (tint) error |= rgba != ((uint32_t)color.r << 24 | (uint32_t)color.g << 16 |
...
static void DrawCases(int tintHearts, int tintMagic) {
    ...
    Reset(); Randomizer_DrawDoubleDefense(&sPlay, NULL);
    CheckStreamWithBorder("Double Defense white border and body", sXlu, sGfx.polyXlu.p,
                          gGiHeartContainerDL, tintHearts, sHeartsColor, 1);
}
int main(void) {
    sCustom = 1; sHeartsChanged = 1; sMagicChanged = 1; DrawCases(1, 1);
    sHeartsColor = (Color_RGB8){245, 41, 79}; sMagicColor = (Color_RGB8){12, 219, 234};
    DrawCases(1, 1); // Fresh CVar values, including rainbow updates, without resource reload.
    sHeartsChanged = 0; DrawCases(0, 1);
    sHeartsChanged = 1; sMagicChanged = 0; DrawCases(1, 0);
    sHeartsChanged = 0; DrawCases(0, 0); // Reset preserves original materials.
    sCustom = 0; sHeartsChanged = 1; sMagicChanged = 1; DrawCases(0, 0);
    sFirstAltDL = gGiHeartPieceDL;
    Reset(); GetItem_DrawXlu01(&sPlay, GID_HEART_PIECE);
    CheckStream("first Alt heart after warm native", sXlu, sGfx.polyXlu.p,
                gGiHeartPieceDL, 1, sHeartsColor);
    sCustom = 0; sFirstAltDL = gGiMagicJarSmallDL;
    Reset(); GetItem_DrawOpa0(&sPlay, GID_MAGIC_SMALL);
    CheckStream("first Alt magic jar after warm native", sOpa, sGfx.polyOpa.p,
                gGiMagicJarSmallDL, 1, sMagicColor);
