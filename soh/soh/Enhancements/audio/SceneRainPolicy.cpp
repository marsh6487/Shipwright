...
SceneRainPolicy ParseSceneRainPolicy(const std::string& bytes, int scene, int setup) {
    ...
    if (scene != 0x5B || setup < 0 || setup > 3 || bytes.size() > 8192) return {};
    ...
    if (!json.is_object() || !json.contains("version") || !json["version"].is_number_integer() || json["version"] != 1 ||
        !json.contains("scene") || json["scene"] != "spot10_scene" ||
        !json.contains("mode") || json["mode"] != "continuous" ||
        !json.contains("density") || !json["density"].is_number_integer() ||
        json["density"] < 1 || json["density"] > 64 ||
        !json.contains("thunder") || !json["thunder"].is_boolean() ||
        !json.contains("diagnostics") || !json["diagnostics"].is_boolean()) return {};
        ...
        if (initData) {
            ...
            if (archive) {
                ...
                if (archive->HasFile(kSceneRainPolicyPath)) {
                    ...
                    if (!policy.valid) SPDLOG_WARN("[rain-probe] rejected policy archive={} setup={}",
                                                   archivePath, setup);
                }
            }
        }
        SPDLOG_INFO("[rain-probe] bind resource={} archive={} setup={} age={} density={} thunder={} diagnostics={}",
                    resourcePath, archivePath, setup, age,
                    policy.density, policy.thunder, policy.diagnostics);
    }
